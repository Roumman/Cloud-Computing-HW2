# Скрипт автоматизирует установку и настройку ClickHouse в Docker на виртуальной машине: устанавливает Docker (при необходимости), запускает контейнер ClickHouse, создаёт таблицу default.logs и пользователя logbroker с правами на работу с этой таблицей.
